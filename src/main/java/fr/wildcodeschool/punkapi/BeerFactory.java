...
public class BeerFactory {
    ...
    public static Beer buildBeer(JsonObject receivedObject) {
        ...
        beer.setName(name);
        // TAG LINE
        String taglineObject = receivedObject.isNull("tagline") ? "" : receivedObject.getString("tagline");
        beer.setTagline(taglineObject);
        // FIRST BREWED
        String firstBrewedObject = receivedObject.isNull("first_brewed") ? "" : receivedObject.getString("first_brewed");
        beer.setFirstBrewed(firstBrewedObject);
        // DESCRIPTION
        ...
        int volumeValueObject = volumeObject.isNull("value") ? 0 : volumeObject.getInt("value");
        String volumeUnitObject = volumeObject.isNull("unit") ? "" : volumeObject.getString("unit");
        ...
        int boilVolumeValueObject = volumeObject.isNull("value") ? 0 : boilVolumeObject.getInt("value");
        String boilVolumeUnitObject = volumeObject.isNull("unit") ? "" : boilVolumeObject.getString("unit");
        beer.setBoilVolume(new BoilVolume(
                boilVolumeValueObject,
                boilVolumeUnitObject
        ));
         ...
         * METHOD
          */
        JsonObject methodObject = receivedObject.isNull("method") ? null : receivedObject.getJsonObject("method");
        ...
        /**
          * INGREDIENTS
          */
        JsonObject receivedIngredients = receivedObject.isNull("ingredients") ? null : receivedObject.getJsonObject("ingredients");
        if(receivedIngredients != null) {
            beer.setIngredients(IngredientFactory.buildIngredientList(receivedIngredients));
        }
